...
// Verbatim from 21st.dev (~/.cursor/skills/component-library/prompts/agent-orb-integration.md).
// Two changes:
//   1. The Perlin noise texture is loaded from /perlin.png (web/public) instead of
//      cdn.21st.dev so the orbs render offline. Same file, downloaded once.
//   2. <Scene> is wrapped in <Suspense> inside the Canvas. drei's useTexture suspends; with
//      R3F 9.7 + React 19.2 and no boundary inside the canvas, the suspension escapes to the
//      outer tree and the R3F root never starts rendering (canvas mounts, zero GL draw calls).
//      This could recur for any R3F scene that uses useTexture/useLoader without a boundary.
...
function Scene({
  ...
  const perlinNoiseTexture = useTexture("/perlin.png")
